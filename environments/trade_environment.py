...
from indicators import add_technical_indicators, calcualte_turbulence
from utils import load_stocks


class TradingStockEnvironment(Env):
    def __init__(
        self,
        ...
        # gym env
        super(TradingStockEnvironment).__init__()
    ...
    def _sell_stock(
        ...
        if self.h[stock_index] > 0:
            sold_shares = (
                min(-sell_action, self.h[stock_index])
                if self.turbulence < self.turbulence_threshold
                else self.h[stock_index]
            )
            self.b += (
                sold_shares
    ...
    def step(self, action) -> tuple[np.ndarray, float, bool, dict]:
        ...
        # perform action
        action = (
            np.floor(action * MAX_SHARES_PER_STOCK)
            if self.turbulence < self.turbulence_threshold
            else np.array([-MAX_SHARES_PER_STOCK] * len(self.ticker))
        )
    ...
    def render(self) -> None:
        ...
        # self.turbulence_df.iloc[:self.current_time+self.window_day_duration].plot()
