...
from .base_agent.models import ActorNetwork, CriticNetwork
from .base_agent.utils import entropy, index_to_one_hot, to_tensor_var
...
class A2C(Agent):
    ...
    def train(self):
        ...
        entropy_loss = torch.mean(entropy(torch.exp(action_log_probs)))
    ...
    def _softmax_action(self, state):
        ...
        return (
            softmax_action_var.data.cpu().numpy()[0]
            if self.use_cuda
            else softmax_action_var.data.numpy()[0]
        )
    ...
    def value(self, state, action):
        ...
        return (
            value_var.data.cpu().numpy()[0]
            if self.use_cuda
            else value_var.data.numpy()[0]
        )
